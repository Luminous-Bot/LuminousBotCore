...
#pragma warning restore IDISP002, IDISP006
        ...
        /// <summary>
        /// Gets the first text channel where the lowest role can send messages to
        /// </summary>
        /// <returns></returns>
        public async Task<SocketTextChannel> GetLowestTextChannelAsync()
        {
            var OrderedTextChannels = this.TextChannels.OrderBy(x => x.Position);
            foreach(var role in this.Roles.OrderBy(x => x.Position))
            {
                foreach(var Channel in OrderedTextChannels)
                {
                    if(Channel.PermissionOverwrites.Count > 0)
                    {
                        ...
                        var value = overwrite.Value;

                        if (value.SendMessages == PermValue.Allow)
                            return Channel;

                        if (value.SendMessages == PermValue.Deny)
                            continue;
