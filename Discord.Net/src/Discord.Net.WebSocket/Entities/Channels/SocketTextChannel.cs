...
namespace Discord.WebSocket
{
    ...
    public class SocketTextChannel : SocketGuildChannel, ITextChannel, ISocketMessageChannel
    {
        ...
        /// <summary>
        /// A Boolean representing if the current user has access to chat in this channel
        /// </summary>
        ...
        /// <summary>
        /// Checks if the current user has permission to send messages into the channel
        /// </summary>
        /// <returns></returns>
        public bool GetChatPermission()
        {
            if (this.PermissionOverwrites.Count == 0)
                return true;
            var ov = this.GetPermissionOverwrite(Discord.CurrentUser);
            if (ov.HasValue)
                return ov.Value.SendMessages == PermValue.Allow ? true : ov.Value.SendMessages == PermValue.Inherit;

            foreach(var role in this.Guild.CurrentUser.Roles.OrderBy(x => x.Position * -1))
            {
                var ovr = this.GetPermissionOverwrite(role);
                if (ovr.HasValue)
                {
                    return ovr.Value.SendMessages == PermValue.Allow ? true : ovr.Value.SendMessages == PermValue.Inherit;
                }
            }
