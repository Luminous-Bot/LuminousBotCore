...
namespace Public_Bot
{
    class ProfileChangeHelper
    {
        ...
        public static async Task<string> BuildImage(string oldpfp, string newpfp)
        {
            ...
            PfpGraphics.DrawString("Old Profile", TitleFont, new SolidBrush(Color.White), new Point(PfpImage.Width / 4, (PfpImage.Height - 276) / 2), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });

            PfpGraphics.DrawString("New Profile", TitleFont, new SolidBrush(Color.White), new Point(PfpImage.Width - (PfpImage.Width / 4), (PfpImage.Height - 276) / 2), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
